//! Async client for the `oxidemx-cursor` GNOME shell extension's
//! window-positioning D-Bus methods. The extension exposes the
//! `org.oxidemx.CursorHelper` service at `/org/oxidemx/CursorHelper`.
//!
...
//! Re-exported via `oxidemx_window::cursor_helper`.
//!
...
const HELPER_SERVICE: &str = "org.oxidemx.CursorHelper";
#[allow(dead_code)]
...
mod tests {
    ...
    // smoke test:
    //
    //   busctl --user call org.oxidemx.CursorHelper \
    //          /org/oxidemx/CursorHelper org.oxidemx.CursorHelper \
    //          MoveOverlay siiii \
    //          "org.oxidemx.overlay" 200 100 -1
    //
