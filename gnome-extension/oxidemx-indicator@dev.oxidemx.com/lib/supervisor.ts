/**
 * Stack health supervisor for the OxideMX indicator.
 *
 ...
 *   - startDaemon()   — `systemctl --user start oxidemx-daemon.service`.
 *   - ensureOverlay() — calls org.oxidemx.Daemon.EnsureOverlayRunning().
 *
...
export interface StackHealth {
    /** org.oxidemx.Daemon is on the session bus AND systemd unit is active. */
    daemonRunning: boolean;
    /** The active device has a connection other than 'off'. */
    deviceLinked: boolean;
    /** org.oxidemx.overlay is on the session bus. */
        ...
        Gio.DBus.session.call(
            ...
            (_conn: Gio.DBusConnection | null, result: Gio.AsyncResult) => {
                try {
                    ...
                    log(`[oxidemx-indicator] supervisor: ListNames failed: ${e}`);
                    resolve([]);
                }
            },
        );
...
async function isSystemdUnitActive(unit: string, cancellable: Gio.Cancellable): Promise<boolean> {
    return new Promise((resolve) => {
        try {
            ...
            log(`[oxidemx-indicator] supervisor: is-active check failed: ${e}`);
            resolve(false);
        }
    });
}
...
export class Supervisor {
    ...
    /** Returns the cached health state. */
    latestHealth(): StackHealth | null {
    ...
    start(): void {
        ...
        this.poll().then((h) => this._notifyIfChanged(h)).catch((e: unknown) => {
            logError(e as object, '[oxidemx-indicator] supervisor initial poll failed');
        });
        ...
        this._timerId = GLib.timeout_add_seconds(
            ...
            () => {
                this.poll().then((h) => this._notifyIfChanged(h)).catch((e: unknown) => {
                    logError(e as object, '[oxidemx-indicator] supervisor poll failed');
                });
    ...
    async poll(): Promise<StackHealth> {
        const [names, daemonUnit] = await Promise.all([
            sessionBusNames(),
            isSystemdUnitActive('oxidemx-daemon.service', this._cancellable),
        ]);
        ...
        const health: StackHealth = {
            daemonRunning: nameSet.has('org.oxidemx.Daemon') && daemonUnit,
            deviceLinked: latest !== null && latest.connection !== 'off',
            overlayRunning: nameSet.has('org.oxidemx.overlay'),
    ...
    /**
     * Start the daemon via `systemctl --user start oxidemx-daemon.service`.
     ...
     * org.oxidemx.Daemon.EnsureOverlayRunning() over D-Bus.
     */
        ...
        if (changed) {
            for (const cb of this._subscribers) {
                try {
                    ...
                    logError(e as object, '[oxidemx-indicator] supervisor subscriber threw');
                }
            }
        }
    }
