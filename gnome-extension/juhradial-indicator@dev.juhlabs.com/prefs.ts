/**
 * JuhRadial Indicator — libadwaita preferences dialog.
 *
...
const VERSION = '0.0.1';
const ISSUE_URL = 'https://github.com/JuhLabs/juhradial-mx/issues';
...
const CLICK_BEHAVIOR_LABELS = [
    'Open JuhRadial popup',
    'Open JuhRadial Settings',
    'Do nothing',
];
...
export default class JuhRadialIndicatorPrefs extends ExtensionPreferences {
    ...
    private _buildDisplayGroup(
        ...
        const showAsRow = new Adw.ActionRow({
            title: 'Show as',
            subtitle: 'Pick what JuhRadial draws in the panel.',
        });
        group.add(showAsRow);
        ...
        const modeValues: DisplayMode[] = ['percent', 'icon', 'both'];
        const modeLabels = ['Percent', 'Icon', 'Both'];
    ...
    private _buildColorsGroup(
        ...
        critRow.connect('notify::value', () => {
            ...
            if (v >= tl) {
                v = tl - 1;
                log('[juhradial-indicator] threshold clamped: critical must be < low');
                critRow.value = v;
            }
        ...
        lowRow.connect('notify::value', () => {
            ...
            if (v <= tc) {
                v = tc + 1;
                log('[juhradial-indicator] threshold clamped: low must be > critical');
                lowRow.value = v;
            }
    ...
    private _buildAboutGroup(): Adw.PreferencesGroup {
        ...
        daemonBtn.connect('clicked', () => {
            // TODO: launch juhradial-settings (daemon management page).
            log('[juhradial-indicator] Open daemon button clicked — not yet wired.');
        });
        ...
        issueBtn.connect('clicked', () => {
            ...
            if (UL !== undefined) {
                ...
                try {
                    ...
                    log(`[juhradial-indicator] Failed to open issue URL: ${e}`);
                }
            }
        });
