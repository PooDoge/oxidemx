...
require_source() {
  ...
  # extension.js is generated by `npm run build` from extension.ts —
  # it is gitignored and may not exist before the first build.
  if [[ ! -f "$EXT_SRC/metadata.json" ]]; then
    err "Missing metadata.json in $EXT_SRC"
    exit 1
  fi
  if [[ ! -f "$EXT_SRC/extension.ts" && ! -f "$EXT_SRC/extension.js" ]]; then
    err "Missing extension.ts (and no compiled extension.js) in $EXT_SRC"
