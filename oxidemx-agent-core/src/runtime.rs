...
//! - Each tool is a thin `CoreTool` delegating to the ToolExecutor seam
//!   (the overlay implements it via `execute_local_tool`; agentd SP1b
//!   will implement it natively).
...
/// delegates execution to the `ToolExecutor` seam.
#[derive(Clone)]
struct CoreTool {
    name: String,
    description: String,
...
impl std::fmt::Debug for CoreTool {
...
impl ToolRuntime for CoreTool {
    async fn execute(&self, args: Value) -> Result<Value, ToolCallError> {
        match self.exec.execute(&self.name, args, &self.sink).await {
            ...
            Err(e) => Err(ToolCallError::RuntimeError(e.into())),
        }
    }
}

impl ToolT for CoreTool {
    fn name(&self) -> &str {
        &self.name
    }
...
impl std::fmt::Debug for OverlayAgent {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        f.debug_struct("OverlayAgent")
...
pub async fn run(
    ...
    let tools = build_tools(mode, &sink, exec);
    ...
    async fn image_vision_live() {
        ...
        impl crate::tool::ToolExecutor for NoTools {
            async fn execute(
                ...
                _n: &str,
                _a: serde_json::Value,
                _s: &Option<crate::events::StreamSink>,
        ...
        let exec: Arc<dyn crate::tool::ToolExecutor> = Arc::new(NoTools);
        let out = super::run(
            crate::mode::AgentMode::Agentic,
            "gemini-2.5-flash",
