...
use crate::widgets::menu::action::MenuAction;
use super::key_bind::KeyBind;
use iced::widget::Button;
use crate::widgets::wrapper::RcElementWrapper;
pub mod icon { pub type Handle = String; }
use iced::theme;
use iced::widget;
...
impl<Message: Clone + 'static> From<iced::Element<'static, Message>> for MenuTree<Message> {
    fn from(value: iced::Element<'static, Message>) -> Self {
...
where
    Element<'a, Message, iced::Theme, Renderer>: From<widget::Button<'a, Message>>,
    Message: std::clone::Clone + 'a,
{
    widget::button(widget::text(label.into()))
        .padding([4, 12])
}
...
>(
    ...
    let key_color = iced::Color::from_rgba(0.5, 0.5, 0.5, 0.8);
        ...
        .flat_map(|(i, item)| {
            ...
            match item {
                MenuItem::Button(label, icon, action) => {
                    ...
                    let mut items = vec![
                        widget::text(l)
                            .ellipsize(iced::advanced::text::Ellipsize::Middle(
                    ...
                    if let Some(icon) = icon {
                        items.insert(0, crate::widgets::icon(icon).size(14).into());
                        items.insert(
                            1,
                            widget::space::horizontal().width(4.0).into(),
                        );
                    }
                ...
                MenuItem::CheckBox(label, icon, value, action) => {
                    ...
                    let mut items = vec![
                        if value {
                            crate::widgets::icon("object-select-symbolic")
                                .size(16)
                    ...
                    if let Some(icon) = icon {
                        ...
                        items.insert(2, crate::widgets::icon(icon).size(14).into());
                    }
                ...
                MenuItem::Folder(label, children) => {
                    ...
                    trees.push(MenuTree::<Message>::with_children(
                        RcElementWrapper::new(iced::Element::from(
                            menu_button::<'static, _>(vec![
                                widget::text(l.clone())
                                    ...
                                    .into(),
                                widget::space::horizontal().into(),
                                crate::widgets::icon("pan-end-symbolic")
                                    .size(16)
                                    .into(),
                            ]),
                        )),
