...
use iced::advanced::{Renderer as IcedRenderer, Widget};
use iced::advanced::layout::{Limits, Node};
use iced::mouse::{self, Cursor};
use iced::advanced::widget::Tree;
use iced::advanced::{Clipboard, Layout, Shell, overlay, renderer};
use iced::{Length, Padding, Point, Rectangle, Size, Vector, event, touch};
...
impl MenuBounds {
    ...
    fn new<Message>(
        ...
        let (children_size, child_positions, child_sizes) =
            get_children_layout::<Message>(menu_tree, renderer, item_width, item_height, tree);
...
pub(crate) struct Menu<'b, Message: std::clone::Clone> {
    ...
    pub(crate) style: Cow<'b, <iced::Theme as StyleSheet>::Style>,
...
impl<'b, Message: Clone + 'static> Menu<'b, Message> {
    pub(crate) fn overlay(self) -> overlay::Element<'b, Message, iced::Theme, iced::Renderer> {
                    ...
                    data.menu_states[if self.is_overlay { 0 } else { self.depth }
                        ...
                        .fold(
                            ...
                            |(menu_root, mut nodes), (_i, ms)| {
                                ...
                                let children_node = ms.layout::<Message>(
                                    overlay_offset,
                                    slice,
                                    renderer,
                                    menu_root,
    ...
    fn update(
        ...
        match event {
            ...
            Mouse(mouse::Event::ButtonPressed(mouse::Button::Left)) | Touch(FingerPressed { .. }) => {
                self.tree.inner.with_data_mut(|data| {
                    data.pressed = true;
                ...
                self.tree.inner.with_data_mut(|state| {
                        ...
                        < 2.0
                    {
                        ...
                        needs_reset |= self.close_condition.click_inside
                            ...
                            && matches!(
                                event,
                                Mouse(mouse::Event::ButtonReleased(mouse::Button::Left)) | Touch(FingerLifted { .. })
                            );
...
impl<Message: Clone + 'static> overlay::Overlay<Message, iced::Theme, iced::Renderer>
    for Menu<'_, Message>
{
...
impl<Message: std::clone::Clone + 'static> Widget<Message, iced::Theme, iced::Renderer>
    for Menu<'_, Message>
{
    ...
    menu.tree.inner.with_data_mut(|state| {
            ...
            .enumerate()
        {
            ...
            if root_bounds.contains(overlay_cursor) {
                ...
                let menu_bounds = MenuBounds::new::<Message>(
                    mt,
                    renderer,
                    menu.item_width,
                    menu.item_height,
    ...
    */
    ...
    menu.tree.inner.with_data_mut(|state| {
        ...
        if matches!(WINDOWING_SYSTEM.get(), Some(WindowingSystem::Wayland)) && remove {
            if let Some(id) = state.popup_id.remove(&window::Id::NONE) {
        ...
        if !item.children.is_empty() && old_index.is_none_or(|i| i != new_index) {
            ...
            let ms = MenuState {
                index: None,
                scroll_offset: 0.0,
                menu_bounds: MenuBounds::new::<Message>(
                    item,
                    renderer,
                    menu.item_width,
                    menu.item_height,
