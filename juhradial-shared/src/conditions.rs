...
mod tests {
    ...
    fn env_set_and_equals() {
        ...
        .eval());
        assert!(!Condition::EnvSet {
            var: "JUHRADIAL_TEST_NEVER_SET_xyzzy".into()
        }
        .eval());

        // Set a known env var for this thread and verify EnvEquals.
        std::env::set_var("JUHRADIAL_TEST_VAR", "hello");
        assert!(Condition::EnvEquals {
            var: "JUHRADIAL_TEST_VAR".into(),
            value: "hello".into()
        }
        .eval());
        assert!(!Condition::EnvEquals {
        ...
        .eval());
        std::env::remove_var("JUHRADIAL_TEST_VAR");
    }

    #[test]
    fn process_running_finds_self() {
        // The current process's comm is `juhradial_shared-…` (cargo
