# OxideMX Indicator + Popup + Settings — Design Document
...
- `oxidemx-window/` (new sibling crate — extracted from overlay-rs)
- D-Bus surface on `daemon/src/dbus/interface.rs`

> **One line:** A GNOME Shell panel indicator that surfaces MX-device battery, opens a daemon-rendered Iced popup with quick toggles/sliders, and acts as the single front-of-house health surface for the OxideMX stack.
...
Give Linux MX users the same at-a-glance battery + quick-controls surface that ships on macOS (Logi Options+) and Windows (Logi Options), with a UI that respects the OxideMX design language and a stack that respects GNOME's "extensions don't own long-running processes" convention.
...
3. **Stack supervisor + health surface** — added 2026-05-24 per the live-conversation clarification. The indicator is the single place a user sees "is OxideMX healthy?" and the single place a user clicks to remediate. This crosses several discrete checks (§3.4) that previously had no UI home.

### In scope

- Phase 0: rename daemon D-Bus name `org.kde.oxidemx` → `org.oxidemx.Daemon`. Add `GetActiveDeviceState`, `ShowPopup`, `DeviceStateChanged` to the wire.
- Phase A: extract `oxidemx-widgets` and `oxidemx-window` sibling crates so `settings-rs`, `overlay-rs`, `popup-rs` share UI primitives instead of copy-pasting.
- Phase 1: ship the new GNOME extension as TypeScript; migrate `oxidemx-cursor` to TypeScript at the same time (shared `tsconfig.json`).
...
```
                        ...
                        │                              │
                        │  ┌─────────────────────────┐ │
                        │  │ oxidemx-indicator     │ │  (NEW, TypeScript)
            ...
            ▼                          ▼                          ▼
  ┌──────────────────┐      ┌────────────────────┐      ┌────────────────────┐
  │   oxidemxd     │      │ oxidemx-overlay  │      │  oxidemx-popup   │
  │  (the daemon)    │◄────►│   (radial menu)    │      │  (Iced popup —     │
  │   - HID++        │      │  Phase A: imports  │      │   daemon-spawned)  │
  │   - org.oxidemx│      │  oxidemx-window  │      │   Phase 3          │
  │     .Daemon      │      │                    │      │  Uses oxidemx-   │
  │   - spawns popup │      │                    │      │   window +         │
  │   - reads        │      │                    │      │   oxidemx-       │
  │     ~/.config/   │      │                    │      │   widgets          │
  ...
  └──────────────────┘      └────────────────────┘      └────────────────────┘
       systemd-managed         session-launched              spawned on-demand
            ...
            ▼                          ▼                          ▼
  ┌──────────────────┐      ┌────────────────────┐      ┌────────────────────┐
  │ oxidemx-shared │      │ oxidemx-widgets  │      │  oxidemx-window  │
...
|---|---|---|---|---|
| **GSettings** | Indicator look + behaviour (display-mode, thresholds, colors, panel target, position, click-behavior, refresh-interval) | gschema XML compiled to GVDB | `org.gnome.shell.extensions.oxidemx-indicator.*` | Extension reads+writes via `Gio.Settings`. `popup-rs` reads via `gio` (gtk-rs) for color/threshold parity. |
| **JSON (config.json)** | Popup contents (mode, host buttons, quick toggles, sliders, volume-on-scroll, etc.) | serde_json on `oxidemx_shared::AppConfig.popup` | `~/.config/oxidemx/config.json` | `settings-rs` reads+writes (Indicator Popup tab). `popup-rs` reads only, watching with `notify`. |

The split is deliberate: thresholds + colors live alongside other GNOME look-and-feel knobs and survive a daemon reinstall; popup contents live with everything else the daemon owns and follow OxideMX backup/export.
...
- `popup` → `org.oxidemx.Daemon.ShowPopup(panel_x, panel_y, panel_w, panel_h)` — daemon spawns `oxidemx-popup` subprocess with the indicator's stage-absolute panel rect on `argv`.
- `settings` → spawn `oxidemx-settings`.
- `none` → no-op.

Right-click always opens a small `PopupMenu` with `Open Settings / Open Extension Preferences / About`.

Popup layout (mirrors `design/oxidemx-indicator/popup.jsx`):
...
Popup window setup (via the new `oxidemx-window` crate):
- `frameless_topmost(APP_ID, Size::new(360, 480))` — no decorations, transparent root, always on top, non-resizable, app_id `org.oxidemx.popup`.
...
**Added 2026-05-24.** The indicator is the single user-visible surface that answers "is the OxideMX stack working right now?"
...
|---|---|---|
| **Daemon process** | `systemctl --user is-active oxidemx-daemon.service` returns "active" AND `org.oxidemx.Daemon` is on the session bus. | Indicator turns red-orange (`is-critical` ring class), tooltip says "OxideMX daemon is not running". Right-click menu adds `Start daemon` → `systemctl --user start oxidemx-daemon.service`. Popup shows a footer "Daemon down" with one-click `Start` button. |
...
/// On miss, spawns `oxidemx-overlay` via tokio::process::Command
...
| `ShowPopup` | method | `(iiii) → ()` `(panel_x, panel_y, panel_w, panel_h)` stage-absolute logical px | daemon (spawns `oxidemx-popup`) | indicator extension on left-click |
| `EnsureOverlayRunning` | method | `() → ()` | daemon (spawns `oxidemx-overlay`) | indicator extension when radial toggle goes ON |
| `DeviceStateChanged` | signal | `(yysss)` | daemon (emitted from `battery.rs` on change) | indicator extension subscribes for push updates |

The cursor-helper extension's existing surface (`org.oxidemx.CursorHelper.{GetCursorPosition, MoveOverlay, RaiseOverlay, ListMonitors, GetFocusedWindowClass}`) is unchanged but reused by the popup.
...
Lives on `oxidemx_shared::config::AppConfig` as `pub popup: PopupConfig`. Serializes to the `popup` key in `config.json`.
...
Catalogs (`QUICK_TOGGLE_CATALOG`, `QUICK_SLIDER_CATALOG`) are `&'static [QuickEntry]` constants in `oxidemx-shared`. Every default id is asserted to live in the catalog by a unit test in Phase 0.
...
```
oxidemx/
├── oxidemx-shared/                  [MODIFIED — add PopupConfig + catalogs]
├── oxidemx-widgets/                 [NEW — Phase A]
│   ├── Cargo.toml
│   └── src/
│       ├── lib.rs
...
└── overlay-rs/
    └── src/                           [MODIFIED — import oxidemx-window]
```
...
1. Top-bar indicator at 30% battery shows yellow glyph + "30%" — matches `design/oxidemx-indicator/index.html#indicator-states`.
...
6. Sidebar in `oxidemx-settings`: "Indicator Popup" appears between "Point & Scroll" and "Haptic Feedback".
...
10. **§3.4 supervisor:** Stopping `systemctl --user stop oxidemx-daemon.service` turns the indicator red within `refresh-interval` seconds and surfaces a `Start daemon` action in the right-click menu and popup footer.
...
- The overlay's iced + canvas rendering — unchanged; only the window-setup + cursor-helper-client lines move into the new `oxidemx-window` crate.
...
- D-Bus name `org.kde.oxidemx` is gone; long-tail of references touched in Phase 0.
- `settings-rs` no longer owns `widgets.rs / style.rs / palette.rs` — they migrate to `oxidemx-widgets`. `settings-rs` keeps a thin `pub use` façade for back-compat.
- `overlay-rs` no longer owns `ext_positioner.rs` — it migrates to `oxidemx-window`.
- `oxidemx-cursor` GNOME extension changes from plain JS to TypeScript; runtime behaviour unchanged.
...
- Original UI spec (preserved as historical record): [`design/oxidemx-indicator/CLAUDE_CODE_PROMPT.md`](design/oxidemx-indicator/CLAUDE_CODE_PROMPT.md)
- Spec deltas (what changed between the prompt and this design): [`design/oxidemx-indicator/SPEC_ADDENDUM.md`](design/oxidemx-indicator/SPEC_ADDENDUM.md)
...
- Visual canvas: `python3 -m http.server -d design/oxidemx-indicator 7000` → `localhost:7000`
