...
struct GoogleCandidate {
    ...
    /// Reason the model stopped (e.g. "STOP", "MAX_TOKENS"); present on
    /// the final streaming chunk. Absent on intermediate chunks and the
    /// blocking path.
...
impl ChatProvider for Google {
    ...
    /// Streaming chat WITH tools for Gemini.
    ///
    /// Upstream ships only the trait default ("Streaming with tools not
    /// supported") for Google — but Gemini's `streamGenerateContent`
    /// endpoint fully supports function calling. This sends the same
    /// tool-aware request as `chat_with_tools` (so tool-call/result
    /// history is preserved) to the SSE endpoint and translates each
    /// chunk into `StreamChunk`s: text parts stream as `Text`, function
    /// calls surface as `ToolUseComplete` (Gemini emits complete
    /// functionCall objects, not partial-JSON deltas), and a final
    /// `Done` carries the stop reason.
    ...
    > {
        ...
        // Tool-aware history + declarations (same as the blocking path).
        let chat_contents = build_google_chat_contents(messages);
        let google_tools = build_google_tools(tools);
        let generation_config = build_generation_config_with_schema(
            self.max_tokens,
        ...
        Ok(create_google_tool_sse_stream(response))
    }
}
...
/// Maps a Gemini `finishReason` to the cross-provider `stop_reason`
/// convention used by `StreamChunk::Done`.
fn map_google_finish_reason(reason: &str) -> String {
    match reason {
...
/// Translates one SSE event from Gemini's `streamGenerateContent` into
/// zero or more `StreamChunk`s, appending them to `out`. `tool_index`
/// is the running count of tool calls across the whole stream (used for
/// the chunk index + unique tool-call ids, and to decide the stop
/// reason).
/// Finds the next SSE event boundary in `buffer`, returning its byte
/// offset and length. Gemini uses `\r\n\r\n`; the bare `\n\n` form is
/// also accepted. Returns the earliest of the two.
...
fn push_google_tool_chunk(
    out: &mut Vec<Result<StreamChunk, LLMError>>,
    tool_index: &mut usize,
    fc: &GoogleFunctionCall,
) {
    let idx = *tool_index;
    *tool_index += 1;
    out.push(Ok(StreamChunk::ToolUseComplete {
        index: idx,
        tool_call: ToolCall {
            id: format!("call_{}_{}", fc.name, idx),
            call_type: "function".to_string(),
            function: FunctionCall {
                name: fc.name.clone(),
...
fn extract_google_stream_chunks(
    event: &str,
    tool_index: &mut usize,
    out: &mut Vec<Result<StreamChunk, LLMError>>,
) {
    for line in event.lines() {
        ...
        // Text + part-level function calls (the current API shape).
        let mut part_fc_count = 0usize;
        for part in &candidate.content.parts {
            if !part.text.is_empty() {
                out.push(Ok(StreamChunk::Text(part.text.clone())));
            }
            if let Some(fc) = &part.function_call {
                push_google_tool_chunk(out, tool_index, fc);
                part_fc_count += 1;
            }
        }
        // Content-level function calls (older/alternate shape) — only if
        // none were found at the part level, to avoid double-emitting.
        if part_fc_count == 0 {
            if let Some(fcs) = &candidate.content.function_calls {
                for fc in fcs {
                    push_google_tool_chunk(out, tool_index, fc);
                }
            } else if let Some(fc) = &candidate.content.function_call {
                push_google_tool_chunk(out, tool_index, fc);
            }
        }

        if let Some(reason) = &candidate.finish_reason {
            let stop_reason = if *tool_index > 0 {
                "tool_use".to_string()
            } else {
                map_google_finish_reason(reason)
            };
...
/// Builds a `StreamChunk` stream from Gemini's SSE `streamGenerateContent`
/// response, mirroring `create_sse_stream`'s UTF-8/`\n\n` buffering but
/// emitting tool-aware chunks. The tool-call index is threaded through
/// the scan state so ids stay unique across the whole stream.
fn create_google_tool_sse_stream(
    ...
    let stream = response
        .bytes_stream()
        .scan(
            (String::new(), Vec::<u8>::new(), 0usize),
            move |(buffer, utf8_buffer, tool_index), chunk| {
                let result = match chunk {
                    Ok(bytes) => {
                        ...
                        let mut results: Vec<Result<StreamChunk, LLMError>> = Vec::new();
                        // Gemini terminates SSE events with `\r\n\r\n`;
                        // accept the bare `\n\n` form too. Split on
                        // whichever boundary comes first.
                        while let Some((pos, delim_len)) = next_sse_event_boundary(buffer) {
                            let event = buffer[..pos].to_string();
                            buffer.drain(..pos + delim_len);
                            extract_google_stream_chunks(&event, tool_index, &mut results);
                        }
                        Some(results)
                    }
