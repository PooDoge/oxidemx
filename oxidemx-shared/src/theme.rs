//! Theme catalogue.
//!
//! Each theme is a JSON file under `oxidemx-shared/themes/`, included
...
//! `~/.local/share/oxidemx/themes/<name>.json` and are loaded by
...
impl Theme {
    ...
    /// user's data dir first (`~/.local/share/oxidemx/themes/`)
...
fn user_themes_dir() -> Option<PathBuf> {
    if let Some(xdg) = std::env::var_os("XDG_DATA_HOME") {
        ...
        if p.is_absolute() {
            return Some(p.join("oxidemx").join("themes"));
        }
    }
