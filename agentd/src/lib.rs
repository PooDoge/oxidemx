...
mod tests {
    ...
    fn transcript_round_trips_per_thread() {
        ...
        s.append("t1", &TranscriptTurn{role:"user".into(), text:"hi".into(), ts:1, attachments:Vec::new()}).unwrap();
        s.append("t1", &TranscriptTurn{role:"assistant".into(), text:"yo".into(), ts:2, attachments:Vec::new()}).unwrap();
        s.append("t2", &TranscriptTurn{role:"user".into(), text:"other".into(), ts:3, attachments:Vec::new()}).unwrap();
