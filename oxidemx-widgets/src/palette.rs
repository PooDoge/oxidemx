//! Design tokens resolved from the active `oxidemx_shared::Theme`.
//!
//! The shared crate stores themes as JSON (Catppuccin Mocha, OxideMX
...
use iced::Color;
use oxidemx_shared::theme::{parse_hex_rgba, Theme, ThemeName};
...
impl Palette {
    /// Build a Palette from a oxidemx_shared theme. Falls back to a
...
pub fn theme_catalogue() -> Vec<(String, String)> {
    ...
    entries.sort_by(|(ka, _), (kb, _)| match (ka.as_str(), kb.as_str()) {
        ...
        ("oxidemx", _) => std::cmp::Ordering::Less,
        (_, "oxidemx") => std::cmp::Ordering::Greater,
        _ => ka.cmp(kb),
    });
    entries
}
