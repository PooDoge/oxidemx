//! SSE transport for the Interactions API: block splitting and event
//! folding into `RoundOutcome`.
//!
//! PORT of `overlay-rs/src/ai_client/sse.rs` (2026-06-12) with two
//! deltas: the overlay's `StreamSink` becomes a plain mpsc text-delta
//! sender, and `stream_round` honors a `CancellationToken` — the gap
//! AutoAgents leaves open (no mid-flight cancellation upstream; spec
//! Part II §7.3). Keep the folding logic in lockstep with the
//! overlay's copy until P1 unification.

use serde_json::json;
use tokio_util::sync::CancellationToken;

use super::INTERACTIONS_URL;

/// Text deltas stream out through this as they arrive.
pub type DeltaSender = tokio::sync::mpsc::Sender<String>;

/// One model round, folded from SSE events (or a blocking response).
#[derive(Default, Debug)]
pub struct RoundOutcome {
    /// Interaction id — the next round's `previous_interaction_id`.
    pub id: Option<String>,
    /// Final status: completed | requires_action | failed | …
    pub status: String,
    /// Accumulated assistant text.
    pub text: String,
    /// Pending function calls: (call_id, name, arguments).
            ...
            if let Ok(v) = serde_json::from_str::<serde_json::Value>(data) {
                ...
                if delta["type"] == "text" {
                    if let Some(t) = delta["text"].as_str() {
                        out.text.push_str(t);
                        if let Some(s) = sink {
                            let _ = s.send(t.to_string()).await;
                        }
                    }
...
/// retries via the blocking path. Cancellation aborts mid-stream
...
mod sse_tests {
    ...
    #[tokio::test]
    async fn text_deltas_forward_through_sink() {
        let (tx, mut rx) = tokio::sync::mpsc::channel(8);
        let mut out = super::RoundOutcome::default();
        let mut fold = super::FnCallFold::default();
        super::apply_sse_event(
            "step.delta",
            r#"{"delta":{"type":"text","text":"hel"}}"#,
            &mut out,
            &mut fold,
            &Some(tx.clone()),
        )
