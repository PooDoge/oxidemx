# SPEC ADDENDUM — JuhRadial Indicator + Popup + Settings
...
|---|---|---|---|
| 1 | Popup window ownership | "Daemon-owned popup, in-process Iced window inside the daemon" | **Separate `juhradial-popup` binary** spawned by daemon's `ShowPopup` D-Bus handler. Keeps `juhradiald` UI-free (~30MB iced/wgpu otherwise loaded into the supervisor). |
| 2 | D-Bus name | `org.juhradial.Daemon` | **Confirmed `org.juhradial.Daemon`** — but this requires a flag-day rename from the legacy `org.kde.juhradialmx`. Rename scope: 5 daemon files, 4 overlay-rs files, settings-rs, packaging desktop file (rename), 3 install scripts, 2 root design docs. Done atomically in Phase 0. |
| 3 | Popup preferences storage | New `~/.config/juhradial/config.toml` (TOML, serde) | **New `popup` field on existing `config.json`** (serde_json, reuses overlay's inotify watcher and `settings-rs/persist.rs` debounce). One file = one reload. |
| 4 | Crate extractions | "Extract `juhradial-widgets` and `juhradial-window`" | **Confirmed — extract both.** New workspace members `juhradial-widgets/` (moves `widgets.rs + style.rs + palette.rs` out of settings-rs) and `juhradial-window/` (moves `ext_positioner.rs` out of overlay-rs and adds `frameless_topmost()` helper from the inlined overlay window setup). Done in Phase A. |
| 5 | GNOME extension language | TypeScript for the new indicator | **TypeScript for *both* extensions.** Existing `juhradial-cursor@dev.juhlabs.com` migrates from plain JS to TS at the same time. Shared `gnome-extension/tsconfig.json`. |
...
**Confirmed scope extension:** The indicator is the single front-of-house surface for "is the JuhRadial stack healthy" and the single click-to-remediate surface. This added a third co-equal responsibility documented as §3.4 in `INDICATOR_DESIGN.md`. Summary:

| Probe | Check | Remediation |
|---|---|---|
| Daemon process | `systemctl --user is-active juhradialmx-daemon.service` + D-Bus name probe | Indicator turns critical-colored. Right-click menu adds `Start daemon`. Popup footer shows `Daemon down — Start`. |
...
| Radial overlay process | When `Radial Overlay` toggle goes ON: D-Bus probe for `org.juhradial.overlay`. If missing: call new daemon method `EnsureOverlayRunning()` which wraps the spawn. | Daemon handles the actual spawn — extension never spawns long-lived processes itself. |
...
| Cursor-helper extension | `Main.extensionManager.lookup('juhradial-cursor@dev.juhlabs.com')` enabled | Popup footer shows one-time warning if missing. |
...
/// spawns `juhradial-overlay-rs` and waits up to 3s for the bus
