# Claude Code prompt — Implement JuhRadial MX indicator + popup + settings

> Paste the contents of this file as the **opening message** to Claude Code in the JuhRadial repository. It is structured as a directive role + scoped plan request + acceptance criteria, not as a code dump. **Do not delete sections** — each one is load-bearing for getting the plan right.
...
- Idiomatic, `clippy`-clean **Rust** with the [**Iced**](https://iced.rs) GUI library for the JuhRadial Settings app, Overlay app, and daemon-rendered popup (everything that isn't the Shell extension)

You favor:

- Small, composable modules over god-files
- GSettings schemas as the single source of truth for **extension** preferences
- TOML/JSON config files (via `serde`) under `~/.config/juhradial/` as the single source of truth for **app** preferences
...
> **How to read this prompt:** the design artifacts, persistence schemas, defaults, section orders, and non-goals are **contracts** — follow them exactly. Code snippets (module shapes, message enums, window flags) are **sketches of intent** — if the existing codebase or the pinned Iced version suggests a cleaner idiom that achieves the same contract, prefer the cleaner idiom and note the deviation in your plan. Don't treat sketch-level details as requirements.

---

## CONTEXT — what this change is

JuhRadial is a Logitech MX-family configuration utility for Linux:

- **JuhRadial daemon** (long-running, **Rust**) — talks to mice over HID/Bluetooth via `hidapi` / `bluer`, exposes state on D-Bus via `zbus`. Owns the in-process Iced popup window too.
- **JuhRadial MX Settings** (existing desktop app, **Rust + Iced**, dark themed, orange/cyan accent) — lives at `settings-rs/` in this repo. Full configuration UI.
- **JuhRadial Radial Menu Overlay** (separate overlay app, **Rust + Iced**) — lives at `overlay-rs/`. The on-screen radial.
- **JuhRadial GNOME Indicator** (this PR's new shell extension, **GJS/TypeScript**) — small status icon in the top panel. Required to be GJS because GNOME Shell extensions cannot be written in any other language.

This work delivers three coupled deliverables:

1. **A GNOME Shell extension** (`gnome-shell-extension-juhradial-indicator/`, **GJS/TypeScript**) that puts a battery/mouse icon in the top bar. Click behavior opens the JuhRadial daemon's popup — the popup itself is NOT rendered by GJS.
...
3. **A new "Indicator Popup" tab inside `settings-rs/`** (the existing JuhRadial MX Settings app, **Rust + Iced**) — sits in the sidebar **directly under "Point & Scroll"** — owns every preference for what the popup displays.
...
> **Critical architectural rule:** The popup is rendered by the JuhRadial daemon (a small Iced window opened on demand) so its theming, accent colors, and quick-settings catalog stay consistent with the rest of JuhRadial — because it literally **shares Rust + Iced widget code with `settings-rs/`**. The GNOME extension only renders the panel indicator icons and proxies clicks over D-Bus.
>
> Two persistence layers, do not cross them:
> - **Extension prefs** (icon display, threshold colors, panel placement) live in **GSettings** under `org.gnome.shell.extensions.juhradial-indicator.*`. The Rust daemon reads these via `gio::Settings` (gtk-rs) so the popup's battery-ring color matches the panel icon color exactly.
> - **Popup prefs** (mode, quick toggles, sliders, host labels, volume-on-scroll, etc.) live in `~/.config/juhradial/config.toml` (or your existing serde format), read+written by `settings-rs/` and read by the daemon popup.

---

## DESIGN ARTIFACTS — read these first

All design artifacts live in `design/juhradial-indicator/` (copied from the visual review project). Open each before planning:

| File | What it shows |
|---|---|
| `design/juhradial-indicator/index.html` | Master canvas — open in a browser. Sections include indicator-in-context, popup variants, indicator state strip, Devices page, **new "Indicator Popup" settings tab (Simple + Power User)**, and **GNOME extension prefs dialog**. |
| `design/juhradial-indicator/styles.css` | Authoritative token + component CSS. Variables prefixed `--jr-*` are the design system; mirror them in your GTK CSS / `.css` files. |
| `design/juhradial-indicator/popup.jsx` | Reference layout for the popup. Spacing, padding, easy-switch segment, quick-toggle list, footer button order. |
| `design/juhradial-indicator/indicator-popup-page.jsx` | The "Indicator Popup" settings tab. Section order is canonical: Mode → Easy-Switch → Quick toggles → Quick sliders (power only) → Interactions. |
| `design/juhradial-indicator/ext-prefs.jsx` | The libadwaita extension prefs. Section order: Preview → Display → Battery level colors → Placement → Behavior → About → Reset. |
| `design/juhradial-indicator/devices-page.jsx` | Existing Devices page in the settings app — for nav reference. The new "Indicator Popup" item goes **between "Point & Scroll" and "Haptic Feedback"** in `APP_NAV`. |
| `design/juhradial-indicator/app-shell.jsx` | Shell chrome (header, sidebar, status bar) reused across every settings page. |
| `design/juhradial-indicator/indicator.jsx` | Top-bar indicator placement + battery glyph rules. |
| `design/juhradial-indicator/screenshots/*.png` | Pre-rendered reference PNGs: `popup-default`, `popup-power`, `popup-dropdown`, `ext-prefs`, `page-simple`, `page-power`, `devices-page`, `indicator-strip`. Use these as the ground-truth images for vision-based comparison against your running builds (some have letterbox padding — compare the content region). |

> When in doubt, the rendered HTML in `index.html` wins. Run `python3 -m http.server -d design/juhradial-indicator 7000` and visit `localhost:7000` to see the source-of-truth.
...
```
gnome-shell-extension-juhradial-indicator/
├── metadata.json               # uuid: juhradial-indicator@<owner>
...
│   ├── battery.ts              # D-Bus client to JuhRadial daemon (org.juhradial.Daemon)
...
- **Click** does NOT open a `PopupMenu` — instead it dispatches per `click-behavior` GSetting: `popup` calls `org.juhradial.Daemon.ShowPopup()` on D-Bus; `settings` spawns `juhradial-settings`; `none` is a no-op. Right-click always opens a minimal menu with "Open Settings", "Open Extension Preferences", "About".
- **Battery state** comes from `org.juhradial.Daemon.GetActiveDeviceState()` (returns `{ battery: int, charging: bool, connection: string, deviceName: string, deviceId: string }`). Poll every `refresh-interval` seconds AND subscribe to the `DeviceStateChanged` signal for instant updates.
...
### Part 2 · Settings app changes (existing JuhRadial MX Settings repo / dir)
...
The popup is a small Iced window the daemon shows on demand when the indicator dispatches `org.juhradial.Daemon.ShowPopup(panel_rect, indicator_rect)`. It is **not** a GJS PopupMenu and it must **not** be styled by the window manager.
...
- **Factor out shared window-shell code** — if `overlay-rs/src/window.rs` isn't already a reusable module, extract it into a sibling crate (`juhradial-window`) as part of this PR so the radial and the popup share one source of truth for window setup. Do **not** copy-paste.
...
- **Reuse the widget library from `settings-rs/`**: extract `widgets/` into a sibling crate (`juhradial-widgets`) and import it from `settings-rs/`, `overlay-rs/`, and the new popup. This is the architectural reason for keeping the popup in Rust.
- **Config loading**: on show, deserialize `~/.config/juhradial/config.toml`'s `[popup]` table into `PopupConfig`. Watch the file with `notify` (the crate) and reload on change so settings tweaks reflect without restarting the daemon.
- **GSettings consumption** (battery thresholds + colors): use `gio` via `gtk-rs`. Subscribe to `changed::*` on `org.gnome.shell.extensions.juhradial-indicator` and re-render. The battery ring color band is computed using the **same** thresholds the extension uses for the panel icon — they must never disagree.
...
- ❌ Do not fork the radial overlay's window-setup code. Share it via a `juhradial-window` crate.

---

## PLAN-FIRST WORKFLOW (tuned for Claude Fable 5)

You are likely running as **Claude Fable 5** in Claude Code. The sections above define the **contract** (what to build, defaults, ordering, non-goals). How you execute is your call — plan the route yourself; the structure below is the checkpoint scheme, not a step-by-step recipe.
...
2. **Surface ambiguities as a numbered question list** at the end of the plan. Wait for answers before starting implementation.

Once the plan is approved, **own the task end-to-end** in one long run rather than stopping after each sub-part:

3. **Implement Part 1 (extension), Part 2 (settings-rs), Part 3 (daemon popup) as three workstreams.** Within a workstream, sequence as you see fit; the only hard ordering constraints are:
   - GSettings schema + `config/popup.rs` (the two persistence contracts) land before anything that reads them
   - `nav.rs` + page stub land before the full `indicator_popup.rs` page
   - The shared `juhradial-window` extraction lands before the popup window that depends on it
   **Delegate independent workstreams to subagents** — Part 1 (GJS) and Part 2 (Rust settings page) share no code and can proceed in parallel. Keep working while subagents run; intervene if one goes off track.
4. **Verify at checkpoints with fresh-context verifier subagents** — not self-review. At the end of each workstream, spawn a subagent with ONLY: (a) the relevant design artifact files, (b) the acceptance criteria below, (c) the diff. Have it report mismatches. Fix before moving on.
5. **Use vision against the design artifacts.** After each UI surface builds and runs:
   - Extension: screenshot the panel indicator (`gnome-screenshot` or grim under a nested `dbus-run-session -- gnome-shell --nested --wayland`)
   - Iced apps: screenshot the running window (`grim`/`spectacle`, or Iced's `window::screenshot` API)
   - Compare against the matching artboard screenshot in `design/juhradial-indicator/` — check spacing, colors, radii, type scale. Iterate until they match. Record each visual diff you fixed.
6. **Keep a lessons file** at `docs/notes/lessons.md` — one entry per correction or confirmed approach (e.g. "iced 0.13 `Task::perform` requires output: Send + 'static — wrap config in Arc"), one-line summary on top. Read it at the start of each session; it survives context resets.
7. **Per workstream completion**, deliver:
   - Screenshots paired with the matching artboard
   - Unit tests:
   ...
   - `cargo clippy --all-targets -- -D warnings` and `cargo fmt --check` clean for any Rust touched
   - Manual-test recipe in the commit/PR description

### Reporting style

- Progress updates and the final summary are **outcome-first**: what shipped, evidence (test results, screenshot comparisons), risks, next step. No internal working notation, no step-by-step narration of your reasoning.
- Do not transcribe or explain your internal reasoning in responses — plans, diffs, and evidence are what the human reviews.
...
- Do you want the daemon popup invoked via D-Bus method (sync), via a `juhradial-popup` subprocess (async, crash-isolated), or via a long-lived `org.juhradial.Daemon.ShowPopup()` IPC into the already-running daemon?
...
- Should the Rust widget library be extracted into a sibling crate (`juhradial-widgets`) right now so the daemon popup can import it, or is that out of scope for this PR?
...
- Log via `console.log('[juhradial]', …)` — never `print`.
...
```
## Sources consulted
- design artifacts opened (one bullet each)
- existing Rust files read in settings-rs/ and overlay-rs/

## Environment facts
- Iced version pinned in settings-rs/Cargo.toml: X.Y.Z
- Existing widgets reusable as-is: [list]
- Existing theme constants reusable as-is: [list]
- Existing config persistence module: path + one line
- Window-setup module in overlay-rs: path + one line

## Plan
- the implementation plan (file tree diffs, task list, D-Bus signatures, schema diff)
