...
- The executor (`oxidemx-harness`, ledger-native, built SP2c) runs the `StepGraph`; each ready step is dispatched to a **fresh** worker (a `Worker` seam; SP2d's real impl = AutoAgents `ReActAgent` + the native tool set from SP1c), with **bounded context** (the step's own brief + only the artifacts it `needs`), not the whole transcript. Ready steps run concurrently (`join_all`); ledger writes are applied sequentially.
...
- **⚠️ SP2d TOOL-GATE CONTRACT (load-bearing — pinned by the SP2c final review).** SP2c's `ApprovalClassifier` is wired *correctly behind a mock*, but the executor classifies the tool calls a worker REPORTS — i.e. **post-hoc**. A naive SP2d `Worker` that runs tools *inside* `run_step` (the natural AutoAgents loop) would execute `rm -rf`/`git push --force` BEFORE the classifier sees it, making the gate advisory. **SP2d MUST make the gate preventive**, via ONE of: **(A, preferred)** the worker dispatches every tool through an executor-owned `ToolDispatcher` that calls `ApprovalClassifier::classify` BEFORE running and returns `Deny`/`NeedsApproval` inline to the agent loop (the AutoAgents tool-trait seam is where this hooks); or **(B)** run the worker in a cwd-confined, network-off OS sandbox (bwrap/seccomp) so post-hoc classification is backstopped by structural confinement (matches `docs/research/approval-guards.md` §1/§5). `StepOutput.tool_calls` is then an **audit echo, not the gate**.
- **SP2d also:** thread a real per-call `ok` into `record_tool_call` (the executor currently records `ok=true`); and gate the trivial-`CompletionPromise` (`verify_cmd:None`) completion path to **non-code steps only** (a `Step.kind == Code` must require a real `Verifier` promise) so a worker can't shortcut a code step to `Done` by reporting no verify command.
...
- **SP2d — real worker + autonomous run.** The REAL `Worker` (AutoAgents `ReActAgent` + native tools) with the **preventive tool-gate (§4.3 ⚠️ contract — MANDATORY)**, the real `CommandRunner` (`tokio::process`), the real `PlannerModel` (local model via `SchemaConstraint::JsonSchema`, cloud on escalate); the SDD flow template (§4.5); autonomous run mode (budget/approval/resume) over the `org.oxidemx.Agent` D-Bus surface (RunTask/TaskStatus/ResumeTask/ReviewApprovals); plus the §4.3 SP2d items (per-call `ok`, code-step verify gate).
