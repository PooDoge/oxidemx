...
## Menu positioning (2026-06-25, live session)
- FIXED + MODULAR (committed): edge-aware Popover positioning — left-align menus to their trigger button + clamp inside the window so a near-edge anchor (e.g. + button, sidebar collapsed) no longer clips. In the shared `menu/popover.rs` Popover, so EVERY menu + future popup inherits it. Replaced Freya `Attached`'s naive centering.
- OPEN (Minor, cosmetic, self-corrects on reopen): model menu FIRST open per session shows excess space / sits high. INSTRUMENTED + RULED OUT our logic: anchor + content_size (320×579.6) + computed off_top (-579.6, menu bottom exactly at pill) are IDENTICAL on first and second open. It's a Freya layout-pass artifact (content laid out during the positioned=false measure frame, not refreshed when the offset resolves; reopen uses the warm layout). Candidate fix needs a forced relayout when `positioned` flips (key the overlay) or a startup pre-warm — both touch the mount/fade (saga zone), defer to a focused pass.
