...
| `docs/superpowers/specs/2026-06-19-sp2-autonomous-coding-harness-design.md` + `docs/research/*` | **SP2**: autonomous coding harness — TaskLedger (resumable manifest) + local-model schema-gated Planner + orchestrator-worker execution (compiler-as-ground-truth, thin workers, hard caps) + SDD-loop flow template + AutoAgents adoptions (keep conductor). | CURRENT — **building SP2a (ledger) foundation; SP2b–d await user review** |
