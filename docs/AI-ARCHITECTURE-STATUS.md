...
| `docs/superpowers/specs/2026-06-19-sp1c-overlay-cutover-design.md` | **SP1c**: complete agentd's REAL turn path (StreamSink+conductor EventSink→EventEmitter unification, agentd-native tools migrated from overlay, history, streaming deltas, run_flow/cancel) THEN overlay → thin `org.oxidemx.Agent` client (delete in-proc path, HostCapability compositor impl). Single combined effort. | CURRENT — **next to build** |
