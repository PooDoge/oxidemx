...
| `docs/superpowers/specs/2026-06-19-sp2-autonomous-coding-harness-design.md` + `docs/research/*` + plans `…sp2a-task-ledger.md`, `…sp2b-planner.md` | **SP2**: autonomous coding harness — TaskLedger + local-model schema-gated Planner + orchestrator-worker execution + SDD-loop flow + AutoAgents adoptions (keep conductor). Schema-gated DAG (§4.7) + risk-tiered approval (§5.1) settled with user. | **SP2a/b/c + SP2d-1 + SP2d-2 BUILT + final-reviewed** (commits b74e744..aac302f). SP2c = approval + harness Executor. **SP2d-1** = `GatedToolExecutor` (preventive gate) + `RealCommandRunner`. **SP2d-2** = harness reconciled to the inline gate (drop classify → block on `NeedsApproval`) + GateLog + `ApproverPrompt` (Attended approvals) + `CoreWorker` (gated `route_turn`) + **chat path now gates tools/triggers approval card** + `PolicyPlannerModel` (cloud-first + Local/Auto). Opus final review: denied tool can't run in either mode (verified). **SP2d-3 next**: construct+drive the Executor over D-Bus (RunTask/TaskStatus/ResumeTask/ReviewApprovals) + SDD-flow + tool_calls capture + from_config classifiers. |
